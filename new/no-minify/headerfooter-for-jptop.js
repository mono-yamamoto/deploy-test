...
/* --------------------------------------------------------------------------------------
 #10235	フェーズ2-2（HTML+CSS+画像遅延読み込み+必須パーツ実装（Webフォントは非同期））
 #10092	フェーズ１（HTML+CSS+画像遅延読み込み）
 #10223, #10223,  #10296
-------------------------------------------------------------------------------------- */
...
(function ($) {
 ...
 var $cookieMessageObject = {};
  ...
  var $hdr_hdr__nav = {};
  var $hdr_hdr__nav_ul = {};
  var $hdr_hdr__nav_ul_li = {};

  var $hdr__fn__search = {};
  var $hdr__fn__search__form = {};
  var $hdr__fn__menu_span_img = {};
...
if (uas.match(ios)) {
    resizeEvent = 'orientationchange'; //#4918 
  }

  flickityProc();

  /*
  	DOMContentLoaded トリガー
  */
  ...
  */
  function isPc() {
    if (uas.indexOf('iPhone')===-1 && uas.indexOf('iPod')===-1 && uas.indexOf('Android')===-1 && uas.indexOf('Mobile')===-1 && uas.indexOf('iPad')===-1 &&  uas.indexOf('Android')===-1) {
  ...
  */
  function pageTop() {
    ...
    if($pageTop.length) {
      var $pageTopBtn = $pageTop.find('a[href="#top"]');
      var $htmlBody = $('html,body');
      window.addEventListener('scroll', pageTopScroll, false);
	...
	/*
		Flickity
	*/
  function flickityProc() {
		var $flickityGallery = document.querySelector('.flickity-gallery');
		var flkty = new Flickity($flickityGallery, {
			// options
			cellAlign: 'left',
			wrapAround: true,
			lazyLoad: true,
			...
			on: {
				ready: function () {
					var $flickityPageDots = document.querySelector('.flickity-page-dots');
					var newNode = document.createElement("li");
					$flickityPageDots.insertBefore(newNode, $flickityPageDots.firstChild);
					var $pause = $flickityPageDots.firstChild;
					$pause.classList.add('flickity-pause');

					var $flickityPageDots_dot = $flickityPageDots.querySelectorAll('.dot');
					$flickityPageDots.addEventListener('click', function (event) {
						// filter for button clicks
						if (!matchesSelector(event.target, '.dot')) {
							return;
						}

						var index = [].slice.call($flickityPageDots_dot).indexOf(event.target);
						var cells = document.querySelectorAll('.flickity-gallery-cell');
						flkty.selectCell(cells[index]);
						event.preventDefault();
					});

					var $flickityPause = document.querySelector('.flickity-pause');
					$flickityPause.addEventListener('click', function (event) {
						var $paused = document.querySelector('.flickity-pause.paused');
						if ($paused === null) {
							$flickityPause.classList.add('paused');
							flkty.stopPlayer();
						} else {
							$flickityPause.classList.remove('paused');
							flkty.playPlayer();
						}
					...
					window.addEventListener(resizeEvent, function (event) {
						var $paused = document.querySelector('.flickity-pause.paused');
						if ($paused === null) {
							flkty.playPlayer();
						}
  ...
  */
  function headerFooter() {
    $body = $('body');
    $header = $("header");
    $hdr_hdr__nav = $(".header .header__nav");
    $hdr_hdr__nav_ul = $(".header .header__nav > ul");
    $hdr_hdr__nav_ul_li = $(".header .header__nav > ul > li");

    $hdr__fn__search = $(".header__function__search");
    $hdr__fn__search__form = $(".header__function__search__form");
    $hdr__fn__menu_span_img = $('.header__function__menu button img');
    ...
    if (decodeURI(location.href).search('search.x') !== -1) {
      ...
      var int__header__function__menu_span_img = setInterval(function () {
        if ($('.header__function__menu button img').length) {
          $hdr__fn__menu_span_img = $('.header__function__menu button img');
          var _src = $hdr__fn__menu_span_img.attr('src');
      ...
      if ($hdr__fn__menu_span_img) {
        var srcX = $hdr__fn__menu_span_img.attr('src');
        if (srcX) {
    ...
    function set__header__function__menu_span_img(imgpass) {
      if (_header__function__menu_span_img_orgPass !== "") {
        $hdr__fn__menu_span_img.attr('src', _header__function__menu_span_img_orgPass + imgpass);
        //console.log($hdr__fn__menu_span_img.attr('src'));
      }
    }

    //sp ハンバーガーメニュー, sp&pc検索窓
    (function () {
      ...
      $(".header__function__menu").on("click", function () {
        ...
        //サーチウィンドウをクローズ
        $hdr__fn__search.removeClass("is-open");
        $hdr__fn__search__form.attr('style', 'display:none');

        $header.toggleClass("is-absolute");
        $hdr_hdr__nav.toggleClass("is-open");
        $contentWrapper.css({
          'position': '',
          'width': '',
          'top': ''
        ...
        if ($hdr_hdr__nav.is(":hidden")) {
          set__header__function__menu_span_img('btn_sp_menu.png');
          //$hdr__fn__menu_span_img.attr('src', '/shared/unchangeable/img/btn_sp_menu.png');
          ...
          //$hdr__fn__menu_span_img.attr('src', '/shared/unchangeable/img/btn_sp_menu_close.png');
      ...
      $(".header__nav__close").on("click", function () {
        $hdr_hdr__nav.removeClass("is-open");
      ...
      $(".header__function__search button").on("click", function () {
        ...
        $hdr__fn__search.toggleClass("is-open");
        $hdr__fn__search__form.attr({
          style: ''
        });

        if ($hdr__fn__search.hasClass('is-open')) { //検索窓 open-close
          $contentWrapper.css({
            'position': '',
            'width': '',
            'top': ''
          ...
          $hdr__fn__search__form.attr('style', 'display:block');
        } else {
          $hdr__fn__search__form.attr('style', 'display:none');
        }
    ...
    (function () {
      ...
      $hdr_hdr__nav_ul_li.each(function () {
      ...
      function droppdownSet(mode) {
        if (mode === 'pc') {
          ...
          if ($hdr_hdr__nav.hasClass('is-open')) { //#4128
            set__header__function__menu_span_img('btn_sp_menu_close.png');
          } else {
            set__header__function__menu_span_img('btn_sp_menu.png');
          }
      ...
      function pcDroppdown() {
        ...
        $hdr_hdr__nav_ul_li.find('.second-wrap').addClass("is-close").attr('style', 'display:none!important');

        $hdr_hdr__nav_ul_li.on(overEvent, function () {
          $this = $(this);
          if (megaTm) {
            clearTimeout(megaTm);
          }
        ...
        $hdr_hdr__nav_ul_li.on(outEvent, function () {
          $this = $(this);
        ...
        function secondWrapOpen(obj) {
          ...
          var $menu = [];
          var $2nd_wrap = $open.find('.second-wrap');
          var $3rd_wrap = $2nd_wrap.find('.third-wrap');
          var $3rd_wrap_active = {};

          $hdr__fn__search.removeClass("is-open"); //search selector remove
          $(".header__function__search > form").hide();

          $open.addClass('is-open');

          $2nd_wrap.removeClass("is-close").attr('style', 'display:block!important');

          if ($3rd_wrap.length) {
            $3rd_wrap.hide();
            $3rd_wrap_active = $3rd_wrap.eq(0);
            $3rd_wrap_active.show(); //third_wrap 一番上のメニューを表示する
			imgDatasrc($3rd_wrap_active);
			  
            $menu = $2nd_wrap.find('.gnavi-2nd-block'); //third_wrap open 処理
            $menu.menuAim({
          ...
          function activateSubmenu(row) {
            var $row = $(row);
			$3rd_wrap_active = $row.find('.third-wrap');
            $3rd_wrap_active.show();
			imgDatasrc($3rd_wrap_active);
          }
      ...
      function pcDroppdownCancell() {
        ...
        $hdr_hdr__nav_ul_li.off(overEvent);
        $hdr_hdr__nav_ul_li.off(outEvent);
      }
	...
	*/
    ...
    (function () {
      ...
      $(".header .header__nav > ul > li >a").focus(function () {
        $hdr_hdr__nav_ul_li.removeClass("is-open");
